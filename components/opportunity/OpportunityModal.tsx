...
export function OpportunityModal({ idea, onClose }: OpportunityModalProps) {
  ...
  const { saved, toggle: toggleSave } = useSavedIdea(
    displayIdea ?? ({} as Idea)
  );
              ...
              <div className="p-6 pb-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-sm border px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest ${DIFFICULTY_COLOR[i.difficulty]}`}
                    >
                      {i.difficulty}
                    </span>
                    <span
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      style={{
                        border: '1px solid var(--border)',
                        color: saved ? 'var(--accent)' : 'var(--text-4)',
                      }}
                ...
                <h2
                  className="text-xl font-bold uppercase tracking-wide leading-snug mb-1"
                  style={{ color: 'var(--text-1)' }}
                >
                  {i.title}
                </h2>
                  ...
                  <dl className="space-y-2.5">
                    {(
                      ...
                      <div key={label} className="flex gap-3">
                        <dt
                          className="w-28 shrink-0 text-[9px] font-bold uppercase tracking-widest pt-[1px]"
                          style={{ color: 'var(--text-4)' }}
                        >
                          {label}
                        </dt>
                ...
                <Section title="Market Intelligence">
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-bold ${SIGNAL_COLOR[i.marketDemand]}`}
                      >
                        {i.marketDemand}
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-bold ${COMPETITION_COLOR[i.competitionLevel]}`}
                      >
                        {i.competitionLevel}
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-bold ${SIGNAL_COLOR[i.monetizationPotential]}`}
                      >
                        {i.monetizationPotential}
                      </p>
                    </div>
                  </div>
                </Section>
