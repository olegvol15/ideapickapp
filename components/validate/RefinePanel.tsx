    ...
    <div className="mt-3 rounded-xl border border-border bg-card px-6 py-5 flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          {hasChanges && (
            <Button
              ...
              size="sm"
              onClick={reset}
              className="gap-1 text-muted-foreground/50 hover:text-muted-foreground hover:bg-transparent"
            >
        ...
        <div className="flex flex-col gap-0.5">
          <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground/60 mb-1">
            Product
          </span>
          <Textarea
            value={product}
        ...
        <div className="grid grid-cols-[80px_1fr] items-start gap-3">
          <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground/60 text-right pr-1 pt-2">
            Audience
          </span>
          <Textarea
            value={audience}
      ...
      {suggestions.length > 0 && (
        <div className="flex flex-col gap-2.5 pt-1 border-t border-border/50">
          ...
          {suggestions.map((s, i) => (
            <div
              key={i}
              className="flex flex-col gap-2 rounded-lg border border-border bg-muted/15 px-4 py-3"
            >
              <div className="flex items-center gap-2 flex-wrap">
                <span
                  className={cn(
                    'text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border shrink-0',
                    SUGGESTION_TYPE_STYLE[s.type]
                  )}
                >
                  {s.type}
                </span>
              ...
              {s.keywords.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {s.keywords.map((kw) => (
                    <span
                      key={kw}
                      className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-muted/60 text-muted-foreground/75 border border-border/60"
                    >
                      {kw}
                    </span>
                  ))}
