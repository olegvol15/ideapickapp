...
type BranchId = keyof typeof STYLE;
interface Branch { id: BranchId; label: string; items: string[]; side: "left" | "right" }

function BranchNode({ branch, align }: { branch: Branch; align: "left" | "right" }) {
  const s = STYLE[branch.id];
  const isRight = align === "right";
  return (
    <div className={`rounded-xl border ${s.border} ${s.bg} px-4 py-3 w-[188px]`}>
      <p className={`text-[8px] font-bold uppercase tracking-widest ${s.label} mb-2 ${isRight ? "text-right" : ""}`}>{branch.label}</p>
      <ul className={`space-y-1 ${isRight ? "flex flex-col items-end" : ""}`}>
        {branch.items.map((item, i) => (
          <li key={i} className={`flex items-start gap-1.5 text-[10px] leading-snug ${isRight ? "flex-row-reverse" : ""}`} style={{ color: "var(--text-2)" }}>
...
export function RoadmapMap({ idea }: { idea: Idea }) {
  ...
  useLayoutEffect(() => {
    function compute() {
      ...
      branchRefs.current.forEach((el, i) => {
        ...
        newPaths.push({ d: `M ${startX} ${cy} C ${midX} ${cy}, ${midX} ${by}, ${endX} ${by}`, stroke: STYLE[branches[i].id].stroke });
      });
      setPaths(newPaths);
    }
    compute();
  ...
  return (
    <div ref={containerRef} className="relative w-full rounded-2xl overflow-hidden" style={{ minHeight: 460, border: "1px solid var(--border)", backgroundColor: "var(--bg-card)" }}>
      <svg className="absolute inset-0 pointer-events-none" width={svgSize.w} height={svgSize.h} style={{ zIndex: 1 }}>
        {paths.map((p, i) => (
          <motion.path key={i} d={p.d} fill="none" stroke={p.stroke} strokeWidth={1.5} strokeOpacity={0.25} strokeDasharray="5 5"
      ...
      <div ref={centerRef} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ zIndex: 10 }}>
        <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.35, ease: "easeOut" }}
          className="rounded-2xl px-5 py-4 text-center" style={{ minWidth: 156, border: "2px solid var(--accent-hi)", backgroundColor: "var(--accent-lo)" }}>
          <p className="text-[7px] font-bold uppercase tracking-[0.25em] mb-1.5" style={{ color: "var(--accent)", opacity: 0.6 }}>MVP</p>
          <p className="text-[13px] font-bold leading-snug" style={{ color: "var(--text-1)" }}>{idea.title.split(/\s*[—–-]\s*/)[0].trim()}</p>
          <div className="mt-2.5 h-px" style={{ backgroundColor: "var(--border)" }} />
          <p className="mt-1.5 text-[9px]" style={{ color: "var(--text-4)" }}>{idea.difficulty} · {idea.marketDemand} demand</p>
        </motion.div>
      </div>

      <div className="absolute top-0 bottom-0 left-6 flex flex-col justify-around py-10" style={{ zIndex: 10 }}>
        {leftBranches.map((branch, i) => (
          <motion.div key={branch.id} ref={el => { branchRefs.current[i] = el; }} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.15 + i * 0.1 }}>
      ...
      <div className="absolute top-0 bottom-0 right-6 flex flex-col justify-around py-10" style={{ zIndex: 10 }}>
        {rightBranches.map((branch, i) => (
          <motion.div key={branch.id} ref={el => { branchRefs.current[leftBranches.length + i] = el; }} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.15 + i * 0.1 }}>
            <BranchNode branch={branch} align="right" />
          </motion.div>
        ))}
      </div>

      <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: "radial-gradient(circle, var(--grid-line) 1px, transparent 1px)", backgroundSize: "28px 28px", zIndex: 0 }} />
    </div>
  );
}
