...
export const VALIDATION_STEP_TYPES = new Set(['reddit-post', 'landing-page', 'interviews', 'survey']);
...
export function getDisplayNextStep(result: EnhancedValidationResult): string | undefined {
...
export function shouldShowPrimaryAction(result: EnhancedValidationResult): boolean {
  ...
  if (isDrop && hasNiche) return false;
  return !isDrop && !!nextStepType && !!STEP_LABEL[nextStepType] &&
    (isProceed ? true : isTest ? VALIDATION_STEP_TYPES.has(nextStepType) : true);
}

export function shouldShowStartBuilding(result: EnhancedValidationResult): boolean {
