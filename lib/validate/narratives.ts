...
export function buildConfidenceBullets(
  result: EnhancedValidationResult
): string[] {
  const m = result.metrics;
  const bullets: string[] = [];
  if (m?.totalApps) bullets.push(`${m.totalApps} apps analyzed`);
  ...
  else if (m?.totalApps) bullets.push('low review concentration');
  if (m?.ratingVariance != null)
    bullets.push(
      m.ratingVariance < 0.1
        ? 'consistent ratings across apps'
        : 'varied ratings across apps'
    );
  return bullets;
}

export function buildFailureBullets(
  result: EnhancedValidationResult,
  ideaCtx?: IdeaContext
): string[] {
  const bullets: string[] = [];
  ...
  if (
    m?.bottom40AvgRating != null &&
    m?.top10AvgRating != null &&
    m.top10AvgRating - m.bottom40AvgRating > 0.5
  )
    bullets.push(
      `Weaker apps average ${m.bottom40AvgRating.toFixed(1)}★ vs ${m.top10AvgRating.toFixed(1)}★ for leaders → the quality gap punishes generic entries immediately`
    );
...
export function buildWhatThisMeans(
  ...
  if (locked)
    ...
    bullets.push(
      'Review concentration creates a moat → you need a defensible differentiator, not just a better interface'
    );
  else if (comp >= 5)
    bullets.push(
      'Competition is real → a clear, specific angle is required before acquisition costs make growth impossible'
    );
  ...
  if (painScore >= 60)
    ...
    bullets.push(
      'Pain signals are moderate → validate the problem with 5 real users before writing code'
    );
  else
    bullets.push(
      idea
  ...
  if (bestEntryStrategy === 'ENTER_VIA_NICHE')
    bullets.push(
      'Broad market is closed → enter via the identified niche to avoid direct competition with entrenched leaders'
    );
  ...
  if (bestEntryStrategy === 'ENTER_VIA_NICHE' && result.nicheAnalysis)
    ...
    strategy =
      'Run a focused validation experiment (landing page or Reddit post) before committing to build.';
...
export function buildActionableSteps(
  ...
  if (bestEntryStrategy === 'ENTER_VIA_NICHE' && nicheAnalysis)
    ...
    positioning =
      'Find a specific user segment that is underserved by existing solutions';
  ...
  if (problem && topCluster)
    ...
    differentiation =
      'Lead with transparent pricing or a lower-cost tier — cost is the top complaint';
  else if (topCluster === 'ux')
    differentiation =
      'Simplify the core workflow — existing apps are frustrating users';
    ...
    differentiation =
      'Optimize for speed and reliability where competitors fall short';
  else
    differentiation =
      'Focus on one core use case and do it significantly better than anyone else';

  let goal: string;
  if (bestEntryStrategy === 'NO_VIABLE_ENTRY')
    goal =
      'Validate whether a narrower problem definition changes the market picture';
  else if (decision === 'drop')
    goal =
      'Confirm real user pain exists before committing any further resources';
  else
    goal =
      'Get 10 people to say they would pay before writing a single line of code';
...
export function buildQuantifiedWinSignals(
  result: EnhancedValidationResult,
  ideaCtx?: IdeaContext
): string[] {
  ...
  if (clusters.length > 0) {
    return clusters.slice(0, 3).map(({ cluster, share }) => {
      ...
      switch (cluster) {
        ...
        default:
          return `${cluster} issues in ${s} of signals`;
      }
    });
  }
...
export function buildScoreExplanation(
  ...
  if (metric === 'competition') {
    ...
    if (level === 'MEDIUM')
      return 'Crowded but not closed — a focused angle can still break through';
  ...
  if (metric === 'pain') {
    ...
    if (level === 'MEDIUM')
      return "Pain exists but isn't acute — validate the problem before building";
    return idea
  ...
  if (level === 'HIGH')
    return isNiche
      ? 'Niche entry has a strong viable path'
      : 'Market gap is real — move before it closes';
  if (level === 'MEDIUM')
    return problem
...
export function buildRefinements(
  ...
  if (bestEntryStrategy === 'ENTER_VIA_NICHE' && nicheAnalysis) {
    const note =
      nicheAnalysis.comparisonNote ??
      'this sub-market shows materially lower competition';
    refinements.push({
      type: 'Positioning',
  ...
  if (topCluster) {
    const clusterMap: Record<
      ...
      {
        title: string;
        reason: (idea: string, share: number, audience?: string) => string;
      }
    > = {
      pricing: {
        title: 'Lead with transparent or one-time pricing',
        reason: (i, s, a) =>
          `${s}% of user signals cite pricing frustration — ${i} can win the segment that's priced out of incumbents${a ? ` (${a})` : ''}`,
      },
      ux: {
        title: 'Cut onboarding to under 2 minutes',
        reason: (i, s, a) =>
          `${s}% of signals cite complexity — ${a ?? 'users'} are abandoning incumbents over friction; ${i} can win by being the simple option`,
      },
      missing_features: {
        title: "Build the workflow feature competitors haven't shipped",
        reason: (i, s) =>
          `${s}% of signals request features incumbents have ignored — first to ship captures these users for ${i}`,
      },
      performance: {
        title: 'Make speed your brand promise',
        reason: (i, s) =>
          `${s}% of signals mention performance issues — ${i} can own reliability as a core differentiator`,
      },
      bugs: {
        title: 'Brand around stability and reliability',
        reason: (i, s) =>
          `${s}% of signals cite bugs — a stable alternative creates real switching incentive for ${i}`,
      },
    };
    ...
    if (entry)
      refinements.push({
        type: 'Feature',
        title: entry.title,
        reason: entry.reason(idea, topCluster.share, audience),
      });
  }
  ...
  if (refinements.length < 2) {
    ...
    if (winTarget?.opportunity)
      refinements.push({
        type: 'Audience',
        title:
          winTarget.opportunity.length > 65
            ? winTarget.opportunity.slice(0, 65).replace(/\s\S*$/, '') + '…'
            : winTarget.opportunity,
        reason:
          winTarget.gap ??
          'A specific audience reduces competition surface and makes your pitch sharper',
      });
  }
