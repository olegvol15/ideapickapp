...
export function getMarketReality(result: EnhancedValidationResult): VerdictConfig {
  ...
  if (locked)
    return { label: 'MARKET IS LOCKED', sublabel: 'Incumbents dominate review share and quality bar', color: 'rose', Icon: XCircle };

  if (isHighComp && concentrated)
    return { label: 'HIGHLY COMPETITIVE', sublabel: 'Saturated — a few players control most reviews', color: 'rose', Icon: XCircle };

  if (isHighComp && !fragmented)
    return { label: 'HIGHLY COMPETITIVE', sublabel: 'Crowded — moderate review concentration', color: 'rose', Icon: XCircle };
  ...
  if (isHighComp)
    return {
      ...
      structureNote: 'High app count but no single player controls the market. Competition is real, but the absence of a dominant incumbent means a well-differentiated entry can still find an audience.',
    };

  if (isMedComp && concentrated)
    return { label: 'MODERATELY COMPETITIVE', sublabel: 'Established leaders hold significant review share', color: 'amber', Icon: AlertTriangle };

  if (isMedComp && !fragmented)
    return { label: 'MODERATELY COMPETITIVE', sublabel: 'Some concentration — clear leaders present', color: 'amber', Icon: AlertTriangle };
  ...
  if (isMedComp)
    return {
      ...
      color: 'amber', Icon: AlertTriangle,
      structureNote: 'Competition is present but the market is fragmented — no incumbent controls it. Entry is possible with a focused angle and clear differentiation.',
    };
  ...
  if (concentrated)
    return {
      ...
      structureNote: 'Few competitors, but existing apps hold strong positions. A quality bar exists even in a less crowded space — polish and trust signals matter.',
    };

  return { label: 'OPEN MARKET', sublabel: 'Low competition, no dominant incumbents', color: 'emerald', Icon: CheckCircle };
}

export function getEntryPossibility(result: EnhancedValidationResult): EntryConfig {
...
export function computeEntryDifficulty(result: EnhancedValidationResult): 'LOW' | 'MEDIUM' | 'HIGH' {
  const locked    = result.metrics?.marketLocked ?? false;
  const dominance = result.metrics?.marketDominance;
...
export function computeDelta(prev: EnhancedValidationResult, curr: EnhancedValidationResult): ValidationDelta {
  return {
