...
function buildKeywords(
  ...
  if (type === 'Positioning') {
    const nicheTerms = niche
      ...
      .filter((w) => w.length > 3)
      .slice(0, 2);
    const clusterTerms = topCluster
      ? (CLUSTER_POSITIONING[topCluster] ?? []).slice(0, 2)
      : [];
    return [...nicheTerms, ...clusterTerms].slice(0, 4);
  }

  if (type === 'Audience') {
    const clusterKws = topCluster
      ? (CLUSTER_AUDIENCE[topCluster] ?? []).slice(0, 3)
      : [];
...
export function buildSuggestions(
  ...
  const ci = result.competitorInsights?.[0];
  const winAudience = result.whereToWin?.find((w) =>
    /segment|niche|audience|trust/i.test(w.title)
  );
  const winAny = result.whereToWin?.[0];
  const winTarget = winAudience ?? winAny;

  if (
    result.nicheAnalysis?.bestKeyword &&
    result.bestEntryStrategy === 'ENTER_VIA_NICHE'
  ) {
    const niche = result.nicheAnalysis.bestKeyword;
    suggestions.push({
      type: 'Positioning',
  ...
  if (winTarget?.opportunity) {
    ...
    const segmentLabel = (CLUSTER_AUDIENCE[topCluster.cluster] ?? [
  ...
  if (ci?.weakness) {
    ...
    const label =
      featureLabelMap[topCluster.cluster] ?? 'a key gap competitors miss';
    suggestions.push({
      type: 'Feature',
  ...
  if (suggestions.length === 0) {
    suggestions.push({
      ...
      reason:
        'The broad market is too competitive — a focused audience reduces competition surface',
      keywords: [
        'solopreneurs',
...
export function buildInputHints(result: EnhancedValidationResult): InputHints {
  ...
  const ci = result.competitorInsights?.[0];
  const winTarget =
    result.whereToWin?.find((w) => /segment|niche|audience/i.test(w.title)) ??
    result.whereToWin?.[0];
  ...
  } else if (topCluster?.cluster === 'ux')
    audience = 'e.g. ADHD users frustrated by complex apps';
  else if (topCluster?.cluster === 'pricing')
    audience = 'e.g. solopreneurs looking for pay-once tools';
  else if (topCluster?.cluster === 'missing_features')
    audience = "e.g. power users whose workflow isn't supported";
  else if (niche) audience = `e.g. people searching for "${niche}"`;

  let coreValue = 'What makes it different? (optional)';
  if (topCluster?.cluster === 'pricing')
    coreValue = 'e.g. transparent pricing, no hidden fees';
  else if (topCluster?.cluster === 'ux')
    coreValue = 'e.g. onboarding under 2 minutes, no learning curve';
  else if (topCluster?.cluster === 'missing_features')
    coreValue = 'e.g. the workflow features incumbents ignore';
  else if (topCluster?.cluster === 'performance')
    coreValue = 'e.g. fast even on low-end devices';
  else if (ci?.weakness)
    coreValue = `e.g. without ${ci.weakness.toLowerCase().replace(/\.$/, '')}`;
